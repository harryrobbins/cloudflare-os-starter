...
- **Turn it on for a workspace.** There are two ways, and both are explicit:
  - In a gadget's **Connections** tab, choose **Connect resource**, then **Web Search** or **Jev decisions**. The dialog explains what the connection grants and has nothing to fill in. The connection is listed there with the gadget's other connections, the gadget's code can use it, and so can the agent in the workspace's chats.
  - The agent asks with `requestConnection`, and you accept the request card in the chat. That connection belongs to **that chat only**: other chats and gadgets do not get it. If the agent later binds it into a gadget (`setGadgetBinding`), that binding appears in the gadget's Connections tab too.
- **Turn it off again.** Delete the binding in the Connections tab, or press **Disconnect** on the accepted card in the chat. Both connectors are *revocable* (`ResourceDescription.revocable`). A chat's agent reaches one only while a gadget binds it or that chat still holds its accepted request. On its next call or turn, an agent that lost it gets no `env` entry and no `webSafe`/`webFetchUnsafe` tools, even in a chat started earlier. Once nothing grants the connection, the Workshop cancels its pending approvals (a held unsafe fetch is never sent) and stops it. Its audit log is kept.
...
**Existing workspaces.** Before this change, both connectors were agent *singletons*, so every workspace of anyone holding the account got them automatically. Those old capsules stop working as soon as the new Workers are deployed. The connectors refuse any Gatekeeper not created for an explicit connection, and the agent's web tools never attach to an ambient capsule. Before its next agent turn (or on open), each workspace re-reads the account's description, sees that it no longer provides a singleton, and retires the capsule: the record is removed, its pending approvals are cancelled and its instance is stopped. The Web Search capsule's storage (its per-workspace query audit log) stays in the workspace Durable Object, but no agent or Gadget can reach it. No data or Durable Object is deleted, and no migration runs. Nothing is granted implicitly any more. A workspace that relied on web search or Jev needs one explicit connection (above). Chats started before then keep the old binding name, but it no longer resolves.
