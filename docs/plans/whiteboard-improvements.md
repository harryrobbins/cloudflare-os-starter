...
**2026-09-24 — connector routes** (branch `feat/wb-connectors`, not yet packed). Curved connectors (cubic Bézier, exact bounds, arc-length labels, tangent arrowheads), editable elbows (segment handles, stored as `segments` offsets from the ports' midpoint) and curve handles (`curve`, chord-relative), keyboard route editing (`E`), Reset route, side pinning by dropping on a side's dot, route-edit ghosts for peers, and automatic elbows that route around sticky notes, shapes, text and icons (A* over a sparse orthogonal visibility grid, length + bend cost, 3,000-state budget, deterministic fallback). The spatial index re-routes only automatic elbows whose route region an obstacle enters or leaves. New fields are additive: `schemaVersion` stays 1 (legacy-fixture test in `test/core/routes.test.js`). Measured on 2,000 shapes + 1,000 elbows: 92 states expanded per search, 0 fallbacks, 7.2 routes recomputed per single-object move (`test/performance/baseline.md`).
