...
// `text(events)` (an ARRAY of TextEvent) and `presence(events)` (an ARRAY of PresenceEvent)
// methods that may return a promise. Text deliveries go through broadcastText and count towards
// the same in-flight limit as operations (they are ordered with them per subscriber).
//
...
export function presenceBytes(ev) {
  ...
  let n = 600; // type, at, clientId (64 chars, UTF-8), name (40 chars, UTF-8), colour, blipId, editing, keys
  if (ev.anchor) n += ev.anchor.length + 4;
  if (ev.head) n += ev.head.length + 4;
  return n;
}
...
export class Hub {
   ...
   * else a fresh one. Throws Error("wave is full") when a new entry would exceed maxSubscribers.
   * @param {any} stub
  ...
  add(stub, clientInfo) {
    ...
    if (existing) {
      ...
      if (this.entries.size >= this.maxSubscribers) throw new Error("wave is full");
  ...
  /**
   * Delivers text events to every subscriber's `text` (the originator included: it advances its
   * textSeq from the echo). Same ordering and back-pressure rules as broadcast. Never rejects.
   * @param {TextEvent[]} events
   * @returns {Promise<void>}
   */
  broadcastText(events) {
    const tasks = [...this.entries.values()].map((entry) => this.#deliver(entry, "text", events));
    return this.#track(Promise.all(tasks).then(() => {}));
  }
  ...
  /** @param {Entry} entry @param {WaveOperationEvent} event */
  #deliverOperation(entry, event) {
    return this.#deliver(entry, "operation", event);
  }

  /**
   * One delivery on the ordered (operation/text) lane of a subscriber.
   * @param {Entry} entry @param {"operation"|"text"} method @param {unknown} payload
   */
  #deliver(entry, method, payload) {
    ...
    let call;
    try {
      call = Promise.resolve(entry.stub[method](payload));
    } catch (e) {
      call = Promise.reject(e);
    }
    return call.then(
