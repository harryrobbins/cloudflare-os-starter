...
describe("icon picker logic", () => {
  ...
  it("opens on a plain I, a keymap command no tool uses", () => {
    ...
    expect(keyAction({ key: "i" })).toEqual({ type: "command", command: "icons" });
    expect(keyAction({ key: "i", shiftKey: true })).not.toEqual({ type: "command", command: "icons" });
