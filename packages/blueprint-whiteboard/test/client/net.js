...
// whiteboard rules and hub (createWhiteboard + Hub over an InMemoryRepository), wired the way
...
export class Net {
  /**
   * @param {{rng?: () => number, minLat?: number, maxLat?: number, fifo?: boolean, coalesceMs?: number}} [options]
   */
  constructor({ rng = Math.random, minLat = 0, maxLat = 50, fifo = true, coalesceMs = 50 } = {}) {
    this.rng = rng;
    this.minLat = minLat;
    this.maxLat = maxLat;
    this.fifo = fifo;
  ...
  connect(name) {
    ...
    return {
      ...
      subscribe: (callback, info) => {
        const epoch = net.epoch;
        // The stub the server keeps: deliveries go down the same FIFO channel as results. The hub
        // disposes it when the entry is removed or replaced, which reaches the client's callback.
        const stub = {
          operation: (ev) => { const c = structuredClone(ev); net.send(down, () => { if (epoch === net.epoch) callback.operation(c); }); },
          presence: (evs) => { const c = structuredClone(evs); net.send(down, () => { if (epoch === net.epoch) callback.presence(c); }); },
          [Symbol.dispose]: () => { net.send(down, () => { if (epoch === net.epoch) callback[Symbol.dispose]?.(); }); },
        };
      ...
      updatePresence: (p) => rpc("updatePresence", [p], async (x) => {
        ...
        return { known, revision: net.board.revisionNow() ?? await net.board.getRevision() };
      }),
