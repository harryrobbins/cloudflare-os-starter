...
| Path | What |
| --- | --- |
| `src/shared/` | The contract: data shapes, wire protocol, limits, sanitisers (`protocol.js`); geometry, text layout and connector routing (`geometry.js`); the object renderer shared by the client and the SVG export (`render.js`); stroke simplification (`simplify.js`); fractional ordering keys (`order.js`); icon and stencil packs (`icons/registry.js` over the checked-in, generated `generated/icon-packs.js`) |
