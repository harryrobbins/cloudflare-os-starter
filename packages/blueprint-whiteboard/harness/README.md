...
- **Restart (stale stub)**: the same restart, and every `gadget` stub handed out so far rejects every call forever. This is what the real platform does to the iframe after a code edit. The client gives up after 3 failed subscribes (or 8 s non-live), reloads its own frame (carrying the viewer's colour, and the name as a fallback for hosts without `gadgetViewer`, in `window.name`) and comes back under the same account name; the reloaded pane gets a fresh stub. A pane with unsaved changes does not reload: it shows the recovery screen (unsaved count, data download, **Reload and lose them**). From a script, `whiteboardStore.replaceTarget(stub)` in a pane reconnects it on a new stub without a reload.
