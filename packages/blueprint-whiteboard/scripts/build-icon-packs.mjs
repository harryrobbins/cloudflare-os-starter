...
//                                        and the emoji data
//   src/client/generated/unicode-data.js emoji and symbols for the picker's "Emoji & symbols" tab
//                                        (client-only; see scripts/unicode/build-unicode.mjs)
//
...
import { UNICODE_OUTPUT, buildUnicode } from "./unicode/build-unicode.mjs";
...
/** @param {any[]} packs @param {string[]} [extraSections] more sections (the emoji data's), appended */
export function renderNotices(packs, extraSections = []) {
  const lines = [
    "# Third-party notices",
    "",
...
export async function buildIconPacks({ ledger } = {}) {
  ...
  const unicode = await buildUnicode();
  return {
    packs,
    unicode,
    files: {
      ...
      [OUTPUTS.notices]: renderNotices(packs, [unicode.notice]),
      [OUTPUTS.unicode]: unicode.text,
    },
    bytes,
  };
}
...
async function main() {
  ...
  const { files, packs, bytes, unicode } = await buildIconPacks();
  const stale = [];
  ...
  if (check) {
    ...
    console.log(`icon packs are current (${count} icons, ${bytes} bytes; ${unicode.data.count} emoji and ${unicode.data.symbolCount} symbols, ${unicode.bytes} bytes)`);
    return;
  }
  console.log(`${stale.length ? "wrote " + stale.join(", ") : "unchanged"}: ${count} icons, ${bytes} bytes of ${GENERATED_BUDGET_BYTES}; ` +
    `${unicode.data.count} emoji (Emoji ${unicode.data.source.emojiVersion}) and ${unicode.data.symbolCount} symbols, ${unicode.bytes} bytes`);
}
