...
**Object fields**: `id, type, x, y, w, h, rot, z, frameId, text, style, version, createdAt, updatedAt, createdBy`, plus `points` for pens, `from, to, fromSide, toSide, routing` for connectors, `packId, iconId` for icons and `language, theme, lineNumbers, wrap, filename` for code blocks.
