...
| `connector` | a line from object `from` to object `to` (neither may be a connector); `fromSide`/`toSide` are `auto`, `top`, `right`, `bottom` or `left`; `routing` is `straight`, `elbow` or `curved`; `text` is an optional label; `style.arrowStart`/`arrowEnd` are `none` or `arrow`. Its box is ignored. Route edits (optional, see "Connector routes" below): `segments` and `curve` |
