...
**Object fields**: `id, type, x, y, w, h, rot, z, frameId, text, style, version, createdAt, updatedAt, createdBy`, plus `points` for pens, `from, to, fromSide, toSide, routing` (and, when edited, `segments` and `curve`) for connectors and `packId, iconId` for icons.
