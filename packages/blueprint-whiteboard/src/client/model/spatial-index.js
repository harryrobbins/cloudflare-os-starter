...
//   connector                 its route's bounds, padded for the stroke and arrowheads, plus the
//                             label box; recomputed whenever either endpoint changes. A connector
//                             whose endpoint is missing has no bounds and is not indexed.
//
...
import {
  rotatedBounds, connectorRoute, pointsBounds, polylineMidpoint, textWidth, rectsIntersect, LINE_HEIGHT,
...
export class SpatialIndex {
  ...
  constructor({ cellSize = CELL_SIZE, largeCells = LARGE_CELLS, debug = false } = {}) {
    ...
    this.endsOf = new Map();
    /** Algorithmic counters for benchmarks and tests (never content). */
    this.stats = { queries: 0, scanned: 0, updates: 0, resets: 0, mismatches: 0 };
  }
  ...
  update(ids, objects = this.objects) {
    ...
    for (const id of ids) {
      ...
      if (o?.type === "connector" || this.endsOf.has(id)) connectors.push(id);
      else this.#index(id);
  ...
  bruteForce(rect) {
    ...
    for (const id in this.objects) {
      const b = effectiveBounds(this.objects[id], this.#resolve);
  ...
  #index(id) {
    ...
    const b = effectiveBounds(o, this.#resolve);
    if (!b) return;
    this.rects.set(id, b);
...
export function applyStoreChange(index, state, change) {
  if (change.kind === "snapshot" || (change.kind === "objects" && !change.objects)) {
    ...
    index.update(change.objects, state.board.objects);
  }
}
