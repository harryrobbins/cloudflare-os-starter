...
import type { ExternalMessageGateway } from "@gadgets/workshop-shared/external-message-gateway";

import type { SearchService } from "./search-client.js";
...
export interface ChatEnv {
  ...
  /**
   * The Workshop's `ExternalMessageGateway` entrypoint, with `props: { source: "chat" }`: how an
   * `@agent` question reaches the asker's own workspace agent (src/do/agent.ts). Generated by
   * `scripts/deploy.ts` when `chat.agentReplies` is on (the default); absent means the deployment has
   * turned agent replies off, and the app says so instead of showing an Agent that never answers.
   * Typed as the contract rather than `Service<...>`: the contract is an interface, not the branded
   * entrypoint class, and the binding is only ever used for that one RPC method.
   */
  readonly WORKSHOP_GATEWAY?: ExternalMessageGateway;
  /**
   * The omni-search Worker's `SearchService` entrypoint, with `props: { source: "chat" }`
   * (docs/plans/omni-search.md, phase 1). Generated by `scripts/deploy.ts` when search is enabled.
   *
   * Optional: absent means search is off, and chat behaves exactly as it does without it -- no outbox
   * rows are written and search stays lexical only. When present, the workspace pushes message,
   * channel and membership changes through a durable outbox (src/do/search-sync.ts) and fuses dense
   * recall into its own search results (src/do/search.ts). Typed as the contract slice in
