...
//
// A hand-kept mirror of `./types.d.ts`, byte for byte: a `.d.ts` cannot be imported as text without
// a bundler rule, and the same file has to be both type-checked at build time and served over RPC at
// run time. `__tests__/vendor.test.ts` compares this string against the raw file, so the two cannot
// drift -- and because `tsc` checks the file, the string is known to be valid TypeScript.
//
// Only the backticks are escaped, which is what keeps the evaluated string identical to the file.
