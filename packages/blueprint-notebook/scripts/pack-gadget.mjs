// Packs dist/ into formats/notebook.gadget, the bundled-format archive the Workshop build installs
// (see cloudflare-os/packages/workshop-backend/format-blueprints/README.md).
//
//   node scripts/pack-gadget.mjs            pack; bumps formats/notebook.json `revision` when code changed
//   node scripts/pack-gadget.mjs --check    exit 1 if formats/notebook.gadget is stale versus dist/
//
...
export const paths = {
  sidecar: join(repo, "formats/notebook.json"),
  archive: join(repo, "formats/notebook.gadget"),
...
async function main() {
  ...
  if (check) {
    ...
    if (stale) {
      console.error("formats/notebook.gadget is stale; run: pnpm --filter blueprint-notebook pack:gadget");
      process.exit(1);
    }
    console.log(`formats/notebook.gadget is current (revision ${sidecar.revision})`);
    return;
  }
  ...
  console.log(`packed formats/notebook.gadget (${bytes.byteLength} bytes, revision ${sidecar.revision})`);
}
