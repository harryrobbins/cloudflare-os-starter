...
#
#   packages/blueprint-kanban/e2e/start-local-platform.sh     # prints PGID and URL, then returns
#   packages/blueprint-kanban/e2e/stop-local-platform.sh      # kills the process group
#
# Env:
...
if [[ -n "${CFOS_REBUILD:-}" || ! -d "$C/node_modules" || ! -f "$C/packages/typed-storage/dist/index.js" \
      || ! -f "$C/packages/workshop-frontend/dist/index.html" ]]; then
