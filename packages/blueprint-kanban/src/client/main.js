...
function showOverlay(text, busy = true) {
  let el = document.getElementById("kanban-connection-overlay");
  if (!el) {
    el = document.createElement("div");
    el.id = "kanban-connection-overlay";
...
if (exportFormatId !== undefined) {
  ...
  // Ask for a name while the board connects behind the dialog, unless a reload carried it over.
  ...
  const onUnrecoverable = () => {
    ...
    if (recent.length >= MAX_AUTO_RELOADS) {
      showOverlay("The board lost its connection. Reload the page.", false);
      return;
    }
  ...
  liveStore.subscribe((state, change) => {
    if (change.kind === "connection" && state.connection === "live") {
      document.getElementById("kanban-connection-overlay")?.remove();
    }
  });
  namePromise.then((result) => {
    liveStore.setViewer(result?.name || "Guest", result?.color || viewer.color);
  });
  /** @type {any} */ (globalThis).kanbanStore = store;
}
