// Packs dist/ into formats/board.gadget, the bundled-format archive the Workshop build installs
// (see cloudflare-os/packages/workshop-backend/format-blueprints/README.md).
//
//   node scripts/pack-gadget.mjs            pack; bumps formats/board.json `revision` when code changed
//   node scripts/pack-gadget.mjs --check    exit 1 if formats/board.gadget is stale versus dist/
//
...
export const paths = {
  sidecar: join(repo, "formats/board.json"),
  archive: join(repo, "formats/board.gadget"),
...
async function main() {
  ...
  if (check) {
    ...
    if (stale) {
      console.error("formats/board.gadget is stale; run: pnpm --filter blueprint-kanban pack:gadget");
      process.exit(1);
    }
    console.log(`formats/board.gadget is current (revision ${sidecar.revision})`);
    return;
  }
  ...
  console.log(`packed formats/board.gadget (${bytes.byteLength} bytes, revision ${sidecar.revision})`);
}
